...
<template>
  <div class="manage-toolbar">
    <div class="manage-toolbar-left">
      ...
      <SfButton type="secondary" size="small" @click="deselectAll" class="toolbar-btn" :disabled="selectedPhotos.length === 0">
        取消全选
      </SfButton>
      <span class="selected-count" v-if="selectedPhotos.length > 0">
        已选择 {{ selectedPhotos.length }} 张照片
      </span>
    </div>
    <div class="manage-toolbar-right">
      <SfButton type="secondary" size="small" @click="$emit('show-add-to-album')" class="toolbar-btn" :disabled="selectedPhotos.length === 0">
        <i class="fas fa-folder-plus"></i> 加入相册
      </SfButton>
      <SfButton type="danger" size="small" @click="$emit('show-delete-selected')" class="toolbar-btn" :disabled="selectedPhotos.length === 0">
