    ...
    <div class="exif-grid">
      ...
      <div class="exif-item" v-if="photo?.metadata?.exif?.rawExif?.BrightnessValue">
        <div class="exif-value">
          {{ formatNumber(photo?.metadata?.exif?.rawExif?.BrightnessValue) }}
        </div>
        <div class="exif-label">{{ $t('photoDetail.exif.ev') }}</div>
      </div>
      <div class="exif-item" v-if="photo?.metadata?.exif?.rawExif?.MeteringMode">
        <div class="exif-value">
          {{ formatMeteringMode(photo?.metadata?.exif?.rawExif?.MeteringMode) }}
        </div>
  ...
  computed: {
    hasExifData() {
      ...
      return !!(
        ...
        exif.focalLength ||
        (exif.rawExif && (exif.rawExif.BrightnessValue || exif.rawExif.MeteringMode))
      )
    },
  },
  methods: {
